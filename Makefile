...
# Make release directory if it does not exist copy OVMF, kernel, and initrd to release directory
release:
	mkdir -p $(RELEASE_DIR)
	cp $(OVMF) $(RELEASE_DIR)
	cp $(KERNEL) $(RELEASE_DIR)
	cp $(INITRD) $(RELEASE_DIR)
	cp $(BUILD_DIR)/verity/* $(RELEASE_DIR)
